...
const PostDetail = () => {
  ...
  const [province_code, province_name] =
    selectedProvince.split('-');
  const [district_code, district_name] =
    selectedDistrict.split('-');
  const [ward_code, ward_name] = selectedWard.split('-');
  ...
  const handleRemoveImage = (
    ...
    if (type === 'string') {
      const arrImageNew = [...images];
      arrImageNew.splice(index, 1);
      setImages(arrImageNew);
    }
  };
  ...
  const {
    ...
    onSubmit: async (values) => {
      ...
      formData.append('province', province_code.trim());
      if (district_code) {
        formData.append('district', district_code.trim());
      }
      if (ward_code) {
        formData.append('ward', ward_code.trim());
      }
      ...
      const { error } = await apiClient2.updatePost(
        Number(id),
        formData,
      );
      if (!error) {
        swal
          .success('Vui lòng đợi bài viết được duyệt !!!')
          .then(() => {
            navigate(path.listPost);
          });
      } else {
        swal.error(
          'Cập nhật bài viết thất bại. Vui lòng kiểm tra lại !!!',
        );
      }
    },
  });
  ...
  useEffect(() => {
    const callApi = async () => {
      const { error, data } =
        await apiClient2.postDetailCurrent(Number(id));
      if (!error) {
        ...
        setFieldValue('content', data.description);
        setFieldValue('price', data.price);
        setFieldValue('acreage', data.acreage);
  ...
  useEffect(() => {
    setFieldValue('images', [...imagesFile, ...images]);
  }, [imagesFile.length, images.length]);
